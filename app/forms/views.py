...
def form_create_view(request):
    ...
    if form_type_id:
        ...
        QuestionFormSet = modelformset_factory(FormQuestion, form=question_form_class, fields=question_fields, extra=1, can_delete=True)
            ...
            if form.is_valid() and question_formset.is_valid():
                ...
                for question_form in question_formset:
                    if question_form.cleaned_data and not question_form.cleaned_data.get('DELETE', False):
                        # Dynamically capture all fields from the form, excluding 'id'
    ...
    if form_type_id:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and question_formset.is_valid():
                ...
                for question_form in question_formset:
                    if question_form.cleaned_data and not question_form.cleaned_data.get('DELETE', False):
                        # Dynamically capture all fields from the form
                        question_data = {field: question_form.cleaned_data[field] for field in question_form.cleaned_data if field != 'DELETE'}
    ...
    if form_type_id:
        ...
        QuestionFormSet = modelformset_factory(FormQuestion, form=OISQuestionForm, fields=['question', 'feature', 'special_characteristic', 'characteristic', 'specifications', 'sample_frequency', 'sample_size', 'done_by'], extra=1, can_delete=True)
    ...
    if form_type_id:
        ...
        # Create a dynamic formset for questions
        QuestionFormSet = modelformset_factory(FormQuestion, extra=1, can_delete=True)
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and question_formset.is_valid():
                ...
                # Save questions dynamically, no hardcoding of fields
                for question_form in question_formset:
                    if question_form.cleaned_data and not question_form.cleaned_data.get('DELETE', False):
                        # Dynamically capture all fields from the form
                        question_data = {field: question_form.cleaned_data[field] for field in question_form.cleaned_data if field != 'DELETE'}
